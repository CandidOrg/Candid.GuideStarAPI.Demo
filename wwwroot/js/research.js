    ...
    watch: {
      ...
      verifiedOrg: function (data) {
        ...
        if (data.financials.most_recent_year_financials.total_revenue != null && !isNaN(data.financials.most_recent_year_financials.total_revenue) && data.financials.most_recent_year_financials.total_revenue > 0) {
          ...
          if (data.financials.most_recent_year_financials.form_type === 'F990') {
            ...
            $('.highcharts-series-1.highcharts-pie-series').hide();
          }
        ...
        if (data.financials.f990_financials) {
        ...
        else if (data.financials.f990ez_financials) {
          for (var i = 0; i < data.financials.f990ez_financials.length; i++) {
            var year = new Date(Date.parse(data.financials.f990ez_financials[i].period_begin)).getFullYear();
            if (typeof year == "number") {
              areaAssData.push([year, data.financials.f990ez_financials[i].total_assets_eoy]);
              areaLiabData.push([year, data.financials.f990ez_financials[i].total_liabilities_eoy]);
            }
          }
        }
        else if (data.financials.pf990_financials) {
          for (var i = 0; i < data.financials.pf990_financials.length; i++) {
            var year = new Date(Date.parse(data.financials.pf990_financials[i].period_begin)).getFullYear();
            if (typeof year == "number") {
              areaAssData.push([year, data.financials.pf990_financials[i].total_assets]);
              areaLiabData.push([year, data.financials.pf990_financials[i].total_liabilities]);
            }
          }
        }
...
function GetYearList() {
  ...
  else if (findocs.finEZs) {
    for (let i = 0; i < findocs.finEZs.length; i++) {
      yearlist.push('z-' + new Date(findocs.finEZs[i].period_end).getFullYear());
      querylist.push(GenerateBalanceSheet(findocs.finEZs[i], 'z'));
    }
  }
  else if (findocs.finPFs) {
    for (let i = 0; i < findocs.finPFs.length; i++) {
      yearlist.push('z-' + new Date(findocs.finPFs[i].period_end).getFullYear());
      querylist.push(GenerateBalanceSheet(findocs.finPFs[i], 'f'));
    }
  }
